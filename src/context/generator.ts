...
export class ContextGenerator {
  ...
  async compileContext(notePaths: string[], options?: {
    ...
    const notes = await Promise.all(
      notePaths.map(p => this.store.readNote(p))
    );
  ...
  async compileActiveContext(): Promise<string> {
    const activeTasks = await this.store.queryNotes({ type: 'task', status: 'active' });
    const nextTasks = await this.store.queryNotes({ type: 'task', status: 'next' });
    ...
    for (const [project, tasks] of byProject) {
      ...
      for (const task of tasks) {
        ...
        const contexts = task.frontmatter.context
          ...
          : '';
        lines.push(`- **[${status}]** ${title}${contexts}`);
  ...
  async generateClaudeMd(projectPath: string): Promise<string> {
    ...
    const referenceDocs = await this.store.queryNotes({ type: 'reference' });

    const lines: string[] = [
      '# CLAUDE.md',
      '',
      '> Auto-generated by Knowledge Vault. Do not edit directly.',
      '',
      activeContext,
      '',
    ];
  ...
  private formatXml(notes: ParsedNote[], opts: { includeMetadata: boolean }): string {
    ...
    for (const note of notes) {
      const title = note.frontmatter.title ?? note.relativePath;
      lines.push(`  <document title="${escapeXml(title)}" path="${escapeXml(note.relativePath)}">`);
